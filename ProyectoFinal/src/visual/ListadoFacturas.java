...
public class ListadoFacturas extends JDialog {
	...
	public ListadoFacturas() {
		...
		{
			{
				String[] columnas = {"ID","Tipo","Persona","Fecha","Cantidad","Precio"};
	...
	public void loadFacturas() {
		...
		for (Factura factura : Tienda.getInstance().getListaFacturas()) {
			...
			rows[3] = factura.getFechaFactura();
			rows[4] = factura.getCantidadxProducto();
			rows[5] = String.format("%.2f", factura.getPrecio());
